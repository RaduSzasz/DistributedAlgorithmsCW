...
start(PL, Id) ->
  receive
    {pl_deliver, _, {bind, Neighbours}} -> wait_task(PL, Id, Neighbours)
  end.
...
wait_task(PL, Id, Neighbours) ->
  receive
    {pl_deliver, _, {task1, start, Max_messages, Timeout}} ->
      ...
      send_messages(task_1, PL, Id, Neighbours, 0, Max_messages, maps:new())
  end.

send_messages(task_1, PL, Id, Neighbours, Max_messages, Max_messages, Counts) when Max_messages /= 0 ->
  receive
    timeout ->
      print_counts(Id, Neighbours, Counts),
      wait_task(PL, Id, Neighbours);
    {pl_deliver, _, {hello, Pid}} ->
      send_messages(task_1, PL, Id, Neighbours, Max_messages, Max_messages, received_message(Pid, Counts))
  end;
send_messages(task_1, PL, Id, Neighbours, Messages_sent, Max_messages, Counts) ->
  receive
    timeout ->
      print_counts(Id, Neighbours, Counts),
      wait_task(PL, Id, Neighbours);
  ...
  send_messages(task_1, PL, Id, Neighbours, Messages_sent + 1, Max_messages, NewCounts).
